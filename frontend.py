        ...
        with ui.tab_panels(tabs, value="Search"):
            with ui.tab_panel("Search"):
                ...
                ui.input(
                    ...
                    on_change=lambda e: [
                        result.set_text(query(e.value)),
                        device_in_question.set_text(result.text),
                    ],
                    validation={"Input too long": lambda value: len(value) < 20},
                ).on("keydown.enter", lambda e: ui.tab_panels(tabs, value="Devices"))
                result = ui.label()
            ...
            with ui.tab_panel("Devices"):
                ...
                with ui.grid(columns=2):
                    ...
                    ui.timer(
                        1.0,
                        lambda: device.set_text(
                            datamodel.devices["SE1936018000231"]["manufacturer"]
                        ),
                    )
                    ui.label("serial:")
                    serial = ui.label()
                    ui.timer(
                        1.0,
                        lambda: serial.set_text(
                            datamodel.devices["SE1936018000231"]["serialnumber"]
                        ),
                    )
